...
Of greater threat to exogeneity are the IMU score components themselves, in particular poverty and age (infant mortality is excluded from other mortality estimates, while physician supply is the direct target of the program and is less apt to influence health other than through actual patient care; see DAG in appendix). If counties just above and below the cutoff are balanced on these components, the capacity for identification improves; therefore, I test this explicitly in sensitivity analyses. However, population age distributions varied widely both between and within eligible and ineligible counties and, as the association between age and mortality is generally nonlinear and discontinuous (e.g. 64 year olds have dramatically higher risk of death than do 18 year olds), these differences may lead to a violation of the exclusion restriction even with balance on raw percentages of elderly residents. To accomodate for this possibility, I used cause-specific crude rates and county-level population estimates for each 10-year age stratum to standardize cause-specific rates to the 2000 U.S. standard million (Anderson & Rosenberg, 1998), creating an additional outcome measure that was comparable across counties with different age distributions. 

A remaining challenge existing only for heart disease and cancer-specific outcomes is how mortality rates are derived in CDC WONDER. Provided rates estimate the number of heart disease or cancer deaths per 10,000 person-years using the entire population as the denominator, rather than the true at-risk set (i.e. those with a current diagnosis of the given condition). This means that rates reflect the probability of dying from cancer or heart disease, conditional on living in that county, rather than the probability of dying from cancer or heart disease, conditional on living in that county *and* having had a diagnosis of cancer or heart disease. Put another way, unless the incidences of cancer and heart disease are comparable for counties near the IMU cutoff, the estimand will reflect differences in base rates rather than better/worse outcomes alone. It is possible to test this assumption for cancer incidence because cancer is a mandatory reportable disease (and I do) but not for heart disease, for which there is no mandated reporting. Instead, I test for equivalence in the prevalence of obesity and physical inactivity, common determinants of heart disease. Table 1 shows baseline demographic characteristics of eligible and non-eligible counties near the cutoff and Figure 1 shows no evidence of sorting.
...
can.inc <- read.csv("C:\\Users\\Owner\\OneDrive\\Documents\\Spring2020\\imu_data\\data\\incd.csv", header = F)
can.inc <- can.inc[5:nrow(can.inc),]
names(can.inc) <-c("state", "fips", "met_obj", "age_adj_inc", "lower_ci", "upper_ci", 
                   "average_annual_count", "recent_trend",
                   "recent_5_year_trend", "lower_ci_trend", "upper_ci_trend")
can.inc <- can.inc[-1,]
can.inc$age_adj_inc <- parse_number(as.character(can.inc$age_adj_inc))
can.inc$county <- str_extract(can.inc$state, "^(.+?),")
can.inc$county <- str_remove_all(can.inc$county, ",")

#merge together
merge.mortb <- can.inc %>%
  select(fips, average_annual_count, age_adj_inc, lower_ci, upper_ci,
         recent_5_year_trend) %>%
  right_join(merge.mortb, by = "fips")
...
tab2 %>%
  kable("latex", align =c("c", "c", "r"), 
        booktabs = TRUE, caption = "Sample characteristics of counties with Index of Medical Underservice (IMU) scores within the median RBC bandwidth and are shown as M(SD)", escape = FALSE) %>%
...
```{r, fig.cap='McCrary Density Test shows no evidence of sorting at the cutoff (p = 0.68)', results='hide', fig.pos = 'h', fig.height=3, fig.width=4}
...
Table 3 depicts the estimated effects of participating in the NHSC program on county-level age-standardized and crude mortality rates. Virtually all estimated coefficients are positive, indicating that participation in the NHSC increased mortality rates, and, in general, estimates derived using the full sample indicate a stronger effect of program participation than do local estimations. Similarly, the absolute effect of program participation is larger for all-cause mortality than for disease-specific mortality, with the largest estimated effect being an absolute [i.e. crude] increase of 359 deaths per 10,000 person-years (95% CI = 317.53 - 401.39) in treated counties near the cutoff versus similarly scored untreated counties. However, particularly for local estimators with RBC confidence intervals, most estimates are imprecise and non-significant.  Age-standardization improves precision somewhat, such that even after bias correction some estimates show statistically significant increases. With the exception of crude all-cause mortality rate, the magnitude and direction of effects are robust against alternate polynomial specifications. Based on graphical inspection of regression plots (available in appendix), quadratic specifications provide a better fit to the data than linear specifications, while cubic specifications do not provide substantial improvement over quadratic models. In balancing parsimony and correct specification, I use local models including quadratic transformations of $X_i$ for further robustness checks. 
...
res.tab %>%
  ...
  kable_styling(row_label_position = "l",latex_options = "hold_position") %>%
  add_header_above(c("", "", "", "95% CI","", "", "95% CI"=2)) %>%
  add_header_above(c(" ", " ","Full Sample" = 2, "Local" = 4)) %>%
  group_rows(start_row = 1, end_row= 9, group_label = "Age-Standardized Rate") %>%
...
I perform placebo tests using a range of false cutoff values of $X_i$ to estimate local average treatment effects at different points along the running variable. To assess balance in covariates around the cutoff, I estimate the LATE of program participation on 2014 percent poverty, percent of population over age 65, county-level median household income, percent of residents who live in rural areas and who are White, Black, or Hispanic, and rates of unemployment, high school graduation, uninsurance, obesity, smoking, and insufficient physical activity using the same model specifications as above. I also assess the effect on placebo outcomes of 1999, 2005, and 2009 age-standardized mortality. 
 
*Robustness to Various Bandwidth Selections*

For both age-standardized and crude all-cause mortality, the RBC estimate chosen at the optimal bandwidth is contained in the 95% confidence intervals of estimated effects for both narrower and wider bandwidths (Figure 2). The magnitude of estimate effects is relatively stable regardless of bandwidth selection, albeit with narrow bandwidths yielding an estimated effect approximately twice as large (an estimated excess 608 deaths per 10,000 person-years for NHSC participating counties). Additionally, while the 95% confidence interval at the optimal bandwidth excludes the null for age-standardized rates, choosing a bandwith 1 point larger or smaller leads to a widening of confidence intervals and inclusion of the null. Crude effects show more sensitivity to choice of bandwidth, with point estimates including both positive and negative effects (Figure 2, top right). In no case do crude RBC confidence intervals exclude the null.

Similar patterns occur in both heart disease and cancer-specific mortality models (Figures 2, rows 2 and 3). The positive estimated effects for age-standardized heart disease and cancer mortality are contained within the RBC 95% confidence interval for all alternative bandwidths excepting the narrowest (absolute distance of 4 points from the cutoff), after which estimated effects rise steeply. All regression discontinuity effects near the optimal bandwidth are nonsignificant but again indicate a higher mortality rate for treated counties. For crude rates, the effect of the NHSC on mortality is consistently null and small in magnitude. 
...
```{r,fig.cap="Age-standardized (left) and crude (right) LATE estimates for all-cause, heart disease, and cancer mortality are robust against choice of bandwidth. LATE is estimated as a local fuzzy RDD with a triangular kernel and second-order polynomial. Dashed line indicates optimal RBC bandwidth, numbers reflect number of observations within a given bandwidth, black dots and blue ribbon represents robust bias-corrected estimate and 95% CI, and pink dots and grey ribbon represents conventional estimate and 95% CI", fig.height=10, fig.width=8}
library(patchwork)
(all.cause.age + all.cause.crude)/(heart.age + heart.crude)/(cancer.age + cancer.crude)+
  plot_annotation(title = "Sensitivity to Bandwidth")

```

*Placebo Outcomes*

Fuzzy regression discontinuities assessing covariate balance and program participation on pretreatment outcomes suggest that participation in the NHSC is not associated with placebo outcomes (Table 4). Encouragingly for the known possible exogeneity violation by IMU score components, NHSC participation is not associated with either percent county residents below the federal poverty line or percent of county residents age 65 or older. Placebo treatment effects for all sociodemographic characteristics are non-significant, although there is some indication that participating counties have higher uninsured rates, smoking rates, and lower percentages of non-Hispanic White residents.

However, the local effects of 2014 NHSC participation on pretreatment mortality rates are significant in several instances, namely, for 1999 age-standardized all-cause mortality ($\tau$ = 119.04, RBC 95% CI = 5.29 - 232.8) and 2009 age-standardized cancer mortality rate ($\tau$ = 89.28, 95% CI = 10.23 - 85.08). There are two possible explanations for this result. One, despite the change to eligbility criteria in 2014, some counties may have participated in the NHSC prior to 2014, meaning that pretreatment "effects" are a byproduct of past eligibility. This is likely more applicable to 2009 than 1999 mortality, given serial correlation of both mortality rate and eligibility determinants. Two, there may be reverse causality such that counties with higher historic mortality are preferentially chosen by the Department of Health and Human Services or by physicians themselves.$\newline$
...
fourc <- rdrobust(y = merge.mortb$age_adjusted_rate_2004c, 
              x = merge.mortb$x_tilde, fuzzy = merge.mortb$treatment,
              c = 0, all=TRUE , masspoints = F, p = 2)
caninc <- rdrobust(y = merge.mortb$age_adj_inc, x = merge.mortb$x_tilde, 
               fuzzy = merge.mortb$treatment, p = 1, masspoints = F)
...
outcome <- c("Percent Poverty", "Percent Over 65",
             ...
             "5 Year Average Cancer Incidence")
...
tau.rdd <- c(pov$Estimate[2],old$Estimate[2]
             ...
             five$Estimate[2], caninc$Estimate[2])
...
ci.robu <- c(paste0(round(pov$ci[3,1],2), " - ", round(pov$ci[3,2],2)),
             ...
             paste0(round(white$ci[3,1],2), " - ", sprintf("%.2f",round(white$ci[3,2],2))),
             paste0(round(black$ci[3,1],2), " - ", round(black$ci[3,2],2)),
             paste0(sprintf("%.2f",round(hisp$ci[3,1],2)), " - ", round(hisp$ci[3,2],2)),
             paste0(sprintf("%.2f",round(rura$ci[3,1],2)), " - ", sprintf("%.2f",round(rura$ci[3,2],2))),
             ...
             paste0(round(caninc$ci[3,1],2), " - ", round(caninc$ci[3,2],2))
             )
...
names(res.tab) <- c("Outcome", "Estimate", "Robust CI")

res.tab %>%
#specify which ones should be bold
  mutate(
     `Robust CI` = cell_spec(`Robust CI`, bold = ifelse(substr(`Robust CI`,1,1)!= "-", T, F)),
     Estimate = cell_spec(Estimate, bold = ifelse(substr(`Robust CI`,1,1)!= "-", T, F))) %>%
  kable("latex", booktabs = T, escape = F, align= c('l', 'r', 'r', 'r', 'r', 'r'),
        caption = "Effects of NHSC Participation on Placebo Outcomes") %>%
  kable_styling(row_label_position = "l",latex_options = "hold_position") %>%
  footnote(general = "Estimates represent tau FRD using a local regression with a triangular kernel")%>%
  add_footnote(label = "Bold face denotes statistical significance (p < 0.05), CI refers to 95% Confidence Interval")  %>%
...
Using the sample of counties below the cutoff, there is consistently no local regression discontinuity effect on age-standardized or crude all-cause mortality when setting placebo thresholds at IMU scores 10 to 20 points below the true threshold (i.e. for IMU scores of 42 - 52; Figure 3 row 1). With the exception of a placebo threshold 6 points below the true threshold, at which the estimated $\tau$ is larger than the 'true' RD effect (excess of 497 deaths versus 161 per 10,000 person-years) and significant, placebo thresholds closer to the true cutoff also show null effects but are larger in magnitude and less stable than those observed further from the true threshold. At no placebo cutoff are estimated effects for crude heart disease mortality significant, but signifcant effects are observed at placebo cutoffs 17 and 6.8 points below the true threshold for age-standardized mortality. As for all-cause mortality, these estimated effects are substantially larger than the true discontinuity effect (Figure 3, row 2). Placebo thresholds for cancer mortality yield similar null effects, again with the exception of a placebo threshold 5 points below the true cutoff for age-standardized rates and 4.3 points below the threshold for crude rate (Figure 3, row 3). 

Given the relative consistency of identifying an effect at false cutoff scores in the range of 4 to 7 points below the tru escore, I perform additional exploration of treatement probability at various IMU scores. The anticipated probabilty of receiving an NHSC physician is approximatley equal for counties with IMU scores between 52 and 58, after which probability begins to drop, reaching zero [as expected] for scores greater than 62. There is no evidence of preferential sorting around these thresholds, and the only locally imbalanced covariate was percent of elderly residents (Analyses provided in appendix). Thus, it is possible that the significant effects at these placebo thresholds are driven by imbalance in age. Alternatively, this may imply an implict threshold, perhaps due to administrative preference for specific counties. 
...
plac.cut %>%
    ...
    geom_ribbon(aes(ymin = lower_bc, ymax = upper_bc), alpha =.3, fill = "#F8766D") +
    theme_bw() + labs(x = "Placebo Cutoff", y = "LATE")+
    geom_hline(yintercept = 0, color = "black")+
...
plac.cutb <- 
purrr::map_df(.x = seq(-21, -.4, .1), 
               .f = ~make_rdd_plot_data_placebo(outcome = data$crude_rate_2015a,
                                               cutoff = .x))
plac.cutb$cutoff <- seq(-21,-.4,.1)
...
plac.cut2 <- 
purrr::map_df(.x = seq(-21, -.4, .1), 
               .f = ~make_rdd_plot_data_placebo(outcome = data$age_adjusted_rate_2015h,
                                               cutoff = .x))
plac.cut2$cutoff <- seq(-21,-.4,.1)
...
plac.cut2b <- 
purrr::map_df(.x = seq(-21, -.4, .1), 
               .f = ~make_rdd_plot_data_placebo(outcome = data$crude_rate_2015h,
                                               cutoff = .x))
plac.cut2b$cutoff <- seq(-21,-.4,.1)
...
```{r, fig.cap = 'LATE at Placebo Thresholds. Age-standardized rates shown on left, crude on right. Ribbon Shows RBC 95% CI', fig.height=10, fig.width=8}
plac.cut3 <- 
purrr::map_df(.x = seq(-21, -.4, .1), 
               .f = ~make_rdd_plot_data_placebo(outcome = data$age_adjusted_rate_2015c,
                                               cutoff = .x))
plac.cut3$cutoff <- seq(-21,-.4,.1)
...
plac.cut3b <- 
purrr::map_df(.x = seq(-21, -.4, .1), 
               .f = ~make_rdd_plot_data_placebo(outcome = data$crude_rate_2015c,
                                               cutoff = .x))
plac.cut3b$cutoff <- seq(-21,-.4,.1)
...
At best, results indicate no effect of participation in the NHSC on county-level mortality; at worst, participation in the NHSC may lead to increased mortality. Nonetheless, a number of potential threats to identification are present in the data, including associations with pretreatment outcomes and significant effects at points along the running variable other than the true cutpoint. I do not find threats to the exclusion restriction in the potential confounding covariates included, but there remains the possibility that other unmeasured variables violated this assumption. The estimand may also have limited generalizability- for instance, I exploit the anamoly in 2014 scoring in order to identify the treatment effect, but it may be that this same anamoly makes 2014 qualitatively different from treatment in other years. For instance, changes in administrative rules necessarily require additional time and investment, perhpas detracting from other program components in ways that adversely affect health. Moreover, the estimand is applicable only to counties near the cutoff, so highly underserved areas with the greatest need may experience different effects.

Additionally, a number of limitations present in the data may have lead to bias and/or measurement error, the most prominent of which was the fact that scores did not come from the NHSC administering body itself. Still, I will note that scores were generated using the same data sources and criteria used by the Department of Health and Human Services, and that generated scores paralleled prior knowledge of participation and eligbility.
...
Calonico, S., Cattaneo, M. D., & Farrell, M. H. (2019). Optimal bandwidth choice for robust bias-corrected inference in regression discontinuity designs. *The Econometrics Journal*, *00*, 1–19. doi: 10.1093/ectj/utz022

Centers for Disease Control and Prevention. (1995). *CDC Wonder*. Retrieved from http://wonder.cdc.gov/

Fagan, E. B., Gibbons, C., Finnegan, S. C., Petterson, S., Peterson, L. E., Phillips Jr, R. L., & Bazemore, A. W. (2015). Family Medicine Graduate Proximity to Their Site of Training. *Family Medicine, 47*(2), 124–130.

Heisler, E. (2017). *The National Health Service Corps* (No. CRS Report R44970). Washington, DC: Congressional Research Service.

Holmes, G. M. (2005). Increasing physician supply in medically underserved areas. *Labour Economics*, 12(5), 697–725. doi: 10.1016/j.labeco.2004.02.003

McCrary, J. (2008). Manipulation of the running variable in the regression discontinuity design: A density test. *Journal of Econometrics*, *142*(2), 698–714.

National Health Service Corps Scholarship Program. , 42 U.S.C. § 2541 (2003).

Parlier, A. B., Galvin, S. L., Thach, S., Kruidenier, D., & Fagan, E. B. (2018). The Road to Rural Primary Care: A Narrative Review of Factors That Help Develop, Recruit, and Retain Rural Primary Care Physicians. *Academic Medicine, 93*(1), 130–140. doi: 10.1097/ACM.0000000000001839

Pathman, D. E., Konrad, T. R., Dann, R., & Koch, G. (2004). Retention of Primary Care Physicians in Rural Health Professional Shortage Areas. *American Journal of Public Health, 94*(10), 1723–1729. doi: 10.2105/AJPH.94.10.1723

Rabinowitz, H. K., Diamond, J. J., Markham, F. W., & Paynter, N. P. (2001). Critical Factors for Designing Programs to Increase the Supply and Retention of Rural Primary Care Physicians. *JAMA, 286*(9), 1041–1048. doi: 10.1001/jama.286.9.1041

Shi, L., Macinko, J., Starfield, B., Politzer, R., Wulu, J., & Xu, J. (2005). Primary Care, Social Inequalities, and All-Cause, Heart Disease, and Cancer Mortality in US Counties, 1990. *American Journal of Public Health, 95*(4), 674–680. doi: 10.2105/AJPH.2003.031716
...
Torrieri, N. (2014). American Community Survey Design and Methodology (January 2014). Available online at: http://Www.census.gov/Acs/Www/Downloads/Survey_methodology/Acs_design_methodology_report_2014.pdf.

U.S. Census Bureau; American Community Survey. (2014). 2014 American Community Survey 1-Year Estimates, Table S0103; generated by Sarah Van Alsten; using American FactFinder; Retrieved from http://factfinder.census.gov

US Department of Health and Human Services. (2014a). Centers for Disease Control and Prevention (CDC) Behavioral Risk Factor Surveillance System Survey Questionnaire. Atlanta: CDC.

US Department of Health and Human Services. (2014b). Health Resources and Services Administration. Area Health Resources Files (AHRF).

U.S. Department of Health and Human Services. (2015, November). Shortage Designation Management System (SDMS): *Manual for Policies and Procedures*. Retrieved from http://contentmanager.med.uvm.edu/docs/sdms_manual_/ahec-documents/sdms_manual_.pdf?sfvrsn=10
...
tab1 %>% kableExtra::kable("latex",booktabs = T, escape = TRUE, align = "r",
  ...
  kableExtra::kable_styling("latex",latex_options = "HOLD_position")%>%
...
![Directed Acyclic Graph depicting causal model of NHSC program and county-level mortality](dag.png)

```{r, fig.height=6, fig.width=7, fig.align='center', eval=FALSE, include=FALSE}
library(DiagrammeR)
#make dag
grViz("
	digraph causal {
...
rdrobust::rdplot(merge.mortb$age_adjusted_rate_2015a, 
                 merge.mortb$x_tilde, fuzzy = merge.mortb$treatment,
                 c = 0, all=TRUE, p = 1, title = "Age-Standardized All-Cause: Linear",
                 x.label = "Transformed IMU Score", y.label = "Age-Standardized Rate", ci = 95, hide = T)
a2<-
rdrobust::rdplot(merge.mortb$age_adjusted_rate_2015a, 
                 merge.mortb$x_tilde, fuzzy = merge.mortb$treatment,
                 c = 0, all=TRUE, p = 2, title = "Age-Standardized All-Cause: Quadratic",
                 x.label = "Transformed IMU Score", y.label = "Age-Standardized Rate", ci = 95, hide = T)
a3<-
rdrobust::rdplot(merge.mortb$age_adjusted_rate_2015a, 
                 merge.mortb$x_tilde, fuzzy = merge.mortb$treatment,
                 c = 0, all=TRUE, p = 3,title = "Age-Standardized All-Cause: Cubic",
                 x.label = "Transformed IMU Score", y.label = "Age-Standardized Rate", ci = 95, hide = T)

h1<-
rdrobust::rdplot(merge.mortb$age_adjusted_rate_2015h, 
                 merge.mortb$x_tilde, fuzzy = merge.mortb$treatment,
                 c = 0, all=TRUE, p = 1, title = "Age-Standardized Heart Disease: Linear",
                 x.label = "Transformed IMU Score", y.label = "Age-Standardized Rate", ci = 95, hide = T)
h2<-
rdrobust::rdplot(merge.mortb$age_adjusted_rate_2015h, 
                 merge.mortb$x_tilde, fuzzy = merge.mortb$treatment,
                 c = 0, all=TRUE, p = 2, title = "Age-Standardized Heart Disease: Quadratic",
                 x.label = "Transformed IMU Score", y.label = "Age-Standardized Rate", ci = 95, hide = T)
h3<-
rdrobust::rdplot(merge.mortb$age_adjusted_rate_2015h, 
                 merge.mortb$x_tilde, fuzzy = merge.mortb$treatment,
                 c = 0, all=TRUE, p = 3,title = "Age-Standardized Heart Disease: Cubic",
                 x.label = "Transformed IMU Score", y.label = "Age-Standardized Rate", ci = 95, hide = T)

c1<- 
rdrobust::rdplot(merge.mortb$age_adjusted_rate_2015c, 
                 merge.mortb$x_tilde, fuzzy = merge.mortb$treatment,
                 c = 0, all=TRUE, p = 1, title = "Age-Standardized Cancer: Linear",
                 x.label = "Transformed IMU Score", y.label = "Age-Standardized Rate", ci = 95, hide = T)
c2<-
rdrobust::rdplot(merge.mortb$age_adjusted_rate_2015c, 
                 merge.mortb$x_tilde, fuzzy = merge.mortb$treatment,
                 c = 0, all=TRUE, p = 2, title = "Age-Standardized Heart Cancer: Quadratic",
                 x.label = "Transformed IMU Score", y.label = "Age-Standardized Rate", ci = 95, hide = T)
c3<-
rdrobust::rdplot(merge.mortb$age_adjusted_rate_2015c, 
                 merge.mortb$x_tilde, fuzzy = merge.mortb$treatment,
                 c = 0, all=TRUE, p = 3,title = "Age-Standardized Heart Cancer: Cubic",
                 x.label = "Transformed IMU Score", y.label = "Age-Standardized Rate", ci = 95, hide = T)
...
```{r, fig.cap='Treatment Probability by IMU Score. Dashed lines, from left to right, show scores of 55, 58, and 62 (the true cutoff)'}
merge.mortb %>%
  ggplot(aes(x = newimu14, y = treatment)) + geom_smooth() +# geom_jitter() +
  geom_vline(xintercept = 58, linetype = "dashed") + geom_vline(xintercept = 55, linetype = "dashed") +
  geom_vline(xintercept = 62, color = "red", linetype = "dashed")+
  theme_bw() + labs(x = "2014 IMU Score", y = "Treatment Probability")
```


```{r}
pov <- rdrobust(y = data$pv_14, 
              x = data$x_tilde, fuzzy = data$treatment,
              c = -6, all=TRUE , masspoints = F, p = 2)
old <- rdrobust(y = data$percent_65_and_older_raw_value_14, 
              x = data$x_tilde, fuzzy = data$treatment,
              c = -6, all=TRUE , masspoints = F, p = 2)
hhinc <- rdrobust(y = data$median_household_income_raw_value_14, 
              x = data$x_tilde, fuzzy = data$treatment,
              c = -6, all=TRUE , masspoints = F, p = 2)
unemp <- rdrobust(y = data$unemployment_rate_2014, 
              x = data$x_tilde, fuzzy = data$treatment,
              c = -6, all=TRUE , masspoints = F, p = 2)
hsgrad <- rdrobust(y = data$high_school_graduation_raw_value_14, 
              x = data$x_tilde, fuzzy = data$treatment,
              c = -6, all=TRUE , masspoints = F, p = 2)
unins <- rdrobust(y = data$uninsured_2014, 
              x = data$x_tilde, fuzzy = data$treatment,
              c = -6, all=TRUE , masspoints = F, p = 2)
obesi <- rdrobust(y = data$adult_obesity_2012_2014, 
              x = data$x_tilde, fuzzy = data$treatment,
              c = -6, all=TRUE , masspoints = F, p = 2)
smoke <- rdrobust(y = data$adult_smoking_value, 
              x = data$x_tilde, fuzzy = data$treatment,
              c = -6, all=TRUE , masspoints = F, p = 2)
inact <- rdrobust(y = data$physical_inactivity_2012_2014, 
              x = data$x_tilde, fuzzy = data$treatment,
              c = -6, all=TRUE , masspoints = F, p = 2)
white <- rdrobust(y = data$percent_non_hispanic_white_raw_value_14, 
              x = data$x_tilde, fuzzy = data$treatment,
              c = -6, all=TRUE , masspoints = F, p = 2)
black <- rdrobust(y = data$percent_non_hispanic_african_american_raw_value_14, 
              x = data$x_tilde, fuzzy = data$treatment,
              c = -6, all=TRUE , masspoints = F, p = 2)
hisp <- rdrobust(y = data$percent_hispanic_raw_value_14, 
              x = data$x_tilde, fuzzy = data$treatment,
              c = -6, all=TRUE , masspoints = F, p = 2)
rura <- rdrobust(y = data$percent_rural_raw_value_14, 
              x = data$x_tilde, fuzzy = data$treatment,
              c = -6, all=TRUE , masspoints = F, p = 2)
caninc <- rdrobust(y = data$age_adj_inc, 
              x = data$x_tilde, fuzzy = data$treatment,
              c = -6, all=TRUE , masspoints = F, p = 1)
...
tau.rdd <- c(pov$Estimate[2],old$Estimate[2]
             ...
             unins$Estimate[2], obesi$Estimate[2], smoke$Estimate[2],
             inact$Estimate[2], white$Estimate[2], black$Estimate[2],
             hisp$Estimate[2], rura$Estimate[2], caninc$Estimate[2])
...
ci.robu <- c(paste0(round(pov$ci[3,1],2), " - ", round(pov$ci[3,2],2)),
             ...
             paste0(sprintf("%.2f",round(hisp$ci[3,1],2)), " - ", round(hisp$ci[3,2],2)),
             paste0(sprintf("%.2f",round(rura$ci[3,1],2)), " - ", sprintf("%.2f",round(rura$ci[3,2],2))),
             paste0(sprintf("%.2f",round(caninc$ci[3,1],2)), " - ", sprintf("%.2f",round(caninc$ci[3,2],2)))
             )
...
#specify which ones should be bold
  mutate(
     ...
     Estimate = cell_spec(Estimate, bold = ifelse(substr(`Robust CI`,1,1)!= "-", T, F))) %>%
  kable("latex", booktabs = T, escape = F, align= c('l', 'r', 'r', 'r', 'r', 'r'),
        caption = "Placebo Effects of NHSC Participation At False Cutoff Score of 56") %>%
  kable_styling(row_label_position = "l",latex_options = "hold_position") %>%
  footnote(general = "Estimates represent tau FRD using a local regression with a triangular kernel")%>%
  add_footnote(label = "Bold face denotes statistical significance (p < 0.05), CI refers to 95% Confidence Interval") 
