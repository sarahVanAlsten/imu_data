...
ci.robu <- c(paste0(round(pov$ci[3,1],2), " - ", round(pov$ci[3,2],2)),
             ...
             paste0(sprintf("%.3f",round(hisp$ci[3,1],2)), " - ", round(hisp$ci[3,2],2)),
             paste0(sprintf("%.3f",round(rura$ci[3,1],2)), " - ", sprintf("%.3f",round(rura$ci[3,2],2))),
...
#specify which ones should be bold
  mutate(
     Robust = cell_spec(Robust, bold = ifelse(substr(Robust,1,1)!= "-", T, F)),
     Estimate = cell_spec(Estimate, bold = ifelse(substr(Robust,1,1)!= "-", T, F))) %>%
...
rddapp::dc_test(`Transformed IMU Score 2014`, cutpoint = -6)
rddapp::dc_test(`Transformed IMU Score 2014`, cutpoint = -8)
