...
However, the local effects of 2014 NHSC participation on pretreatment mortality rates were significant in several instances, namely, for 1999 age-standardized all cause mortality ($\tau$ = 119.04, RBC 95% CI = 5.29 - 232.8) and 2009 age-standardized cancer mortality rate ($\tau$ = 89.28, 95% CI = 10.23 - 85.08). There are two possible explanations for this result. One, despite the change to eligbility criteria in 2014, some counties may have participated in the NHSC prior to 2014, meaning that pretreatment "effects" are a byproduct of past eligibility. This is likely more applicable to 2009 than 1999 mortality, given serial correlation of both mortality rate and eligibility determinants. Two, there may be reverse causality such that counties with higher historica mortality are preferentially chosen by the Department of Health and Human Services or by physicians themselves to receive funding.$\newline$

*Placebo Cutoffs*

Using the sample of counties below the cutoff, there was consistently no local regression discontinuity effect on age-standardized or crude all cause mortality when setting placebo thresholds at IMU scores 10 to 20 points below the true threshold (i.e. for IMU scores of 42 - 52; Figure 9). With the exception of a placebo threshold 6 points below the true threshold, at which the estimated $\tau$ was larger than the 'true' RD effect (excess of 497 deaths versus 161 per 10,000 person-years) and significant, placebo thresholds closer to the true cutoff also showed null effects but were larger in magnitude and less stable than those observed further from the true threshold. At no placebo cutoff were estimated effects for crude heart disease mortality significant (Figure 10, bottom), but signifcant effects were observed at placebo cutoffs 17 and 6.8 points below the true threshold (Figure 10, top). As for all cause mortality, these estimated effects were substantially larger than the true discontinuity effect, indicating an excess of 166 and 302 versus 55 deaths from heart disease per 10,000 person-years, respectively. As depicted in Figure 11, placebo thresholds for both age-standardized and crude cancer mortality yielded similar null effects, again with the exception of a placebo threshold 5 points below the true cutoff for age-standardized rates ($\tau$ = 120.88) and 4.3 points below the threshold for crude rates ($\tau$ = 1026.53). 

Given the relative consistency of identifying an effect at false cutoff scores in the range of 55 - 58 (4 to 7 points below an IMU score of 62), I performed additional exploration of treatement probability at various IMU scores (Figure 12). The anticipated probabilty of receiving an NHSC physician is approximatley equal for counties with IMU scores between 52 and 58, after which probability begins to drop, reaching zero [as expected] for scores greater than 62. There is no evidence of preferential sorting around these thresholds, and the only locally imbalanced covariate was percent of elderly residents (Table 6 in supplement). Thus, it is possible that the significant effects at these placebo thresholds are driven by imbalance in age. Alternatively, this may imply an implict threshold, perhaps due to administrative preference for specific counties, remains an area for future investigation. 
$\newline$

*Conclusion and Limitations*

At best, results indicated no effect of participation in the NHSC on county-level mortality; at worst, participation in the NHSC may lead to increased mortality for counties with scores near the cutoff. Nonetheless, a number of potential threats to identification were present in the data, including associations with pretreatment outcomes and significant effects at points along the running variable other than the true cutpoint. I did not find threats to the exclusion restriction in the potential confounding covariates included, but there remains the possibility that other unmeasured variables violated this assumption. The estimand may also have limited generalizable- for instance, I exploit the anamoly in 2014 scoring in order to identify the treatment effect, but it may be that this same anamoly makes 2014 qualitatively different from treatment in other years. For instance, changes in administrative rules necessarily require additional time and investment, perhpas detracting from other program components in ways that adversely affect health. Moreover, the estimand is applicable only to counties near the cutoff, so highly underserved areas with the greatest need likely experience different effects.
...
```{r, fig.cap="LATE at placebo cutoffs. Ribbon Shows RBC 95% Confidence Interval"}
...
plac.cut %>%
    ...
    ggtitle("2015 age-standardized All-Cause Mortality")+
...
plac.cutb %>%
    ...
    xlim(c(-21, 2))+ theme(panel.grid.minor.x = element_blank()) + 
    coord_cartesian(ylim = c(-1000, 2000))+
    ggtitle("2015 Crude All-Cause Mortality")
...
```{r, fig.cap = 'LATE at Placebo Thresholds. Ribbon Shows RBC 95% CI'}
plac.cut2 <- 
purrr::map_df(.x = seq(-21, -.4, .1), 
...
pp3<-
  plac.cut2 %>%
    ...
    xlim(c(-21, 2))+ theme(panel.grid.minor.x = element_blank()) + 
    coord_cartesian(ylim = c(-1000, 2000))+
    ggtitle("2015 age-standardized Heart Disease Mortality")+
  theme(axis.text.x = element_blank())
...
pp4<-
  plac.cut2b %>%
    ...
    xlim(c(-21, 2))+ theme(panel.grid.minor.x = element_blank()) + 
    coord_cartesian(ylim = c(-1000, 2000))+
    ggtitle("2015 Crude Heart Disease Mortality")
...
pp5<-
  plac.cut3 %>%
    ...
    ggtitle("2015 age-standardized Cancer Mortality")+
    theme(axis.text.x = element_blank())
...
tau.rdd <- c(pov$Estimate[2],old$Estimate[2]
             ...
             unins$Estimate[2], obesi$Estimate[2], smoke$Estimate[2],
             inact$Estimate[2], white$Estimate[2], black$Estimate[2],
             hisp$Estimate[2], rura$Estimate[2])
