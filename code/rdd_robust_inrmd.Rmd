...
$$
  ...
  In addition to the monotonicity, as discussed above, the ability to identify the RDD effect as the local average treatment effect for the subpopulation of compliers (i.e. counties which are not perpetually enrolled- or never enrolled- in the program) which have IMU scores near the cutoff hinges upon several additional assumptions, namely, continuity in potential outcomes and potential treatment near the threshold, the relevance of $Z_i$, and the exclusion restriction. Supplementary Table
...
\end{aligned}
$$
  The second stage is estimated by equation 3, where $\hat{D_i}$ represents the fitted values of equationn 1, and the various functions of $\tilde{X_i}\hat{D_i}$ represents fitted values from various functional forms of equation 2, with $f_1(\tilde{X_i}) = \tilde{X_i}^p$.
...
names(weak.int) <- c("Outcome","F-Statisic", "P-value", "F-Statisic", "P-value", "F-Statisic", "P-value")
...
weak.int %>%
  ...
  add_header_above(c("Outcome", "Polynomial Order"=6)) %>%
...
outcome <- c("Median HH Income", "Unemployment", "High School Graduation",
             ...
             "1999 Heart Disease Mortality", "2004 Heart Disease Mortality", "2009 Heart Disease Mortality",
